...
# 保存模型和缩放器
joblib.dump(best_rf_model, 'best_rf_model_chinese.pkl')
joblib.dump(scaler, 'rf_feature_scaler_chinese.pkl')
print("\n模型和缩放器已保存.")
...
set_plot_labels('随机森林贝叶斯优化: 实际值 vs 预测值', '实际韧性值 (J/cm²)', '预测韧性值 (J/cm²)')
...
plt.tight_layout()
plt.savefig('rf_bayesian_scatter_chinese.png')
plt.close()
...
set_plot_labels('随机森林贝叶斯优化: 残差分析', '预测韧性值 (J/cm²)', '残差 (实际值 - 预测值)')
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('rf_bayesian_residuals_chinese.png')
plt.close()
...
# 选取前10个最重要的特征
n_features_to_plot = min(10, len(feature_columns))

plt.figure(figsize=(12, 8))
colors = plt.cm.viridis(np.linspace(0, 0.8, n_features_to_plot))
bars = plt.barh(range(n_features_to_plot), 
         importances[indices[:n_features_to_plot]], 
         align='center',
         color=colors)
plt.yticks(range(n_features_to_plot), 
          [feature_columns[i] for i in indices[:n_features_to_plot]])
set_plot_labels('特征重要性分析', '相对重要性', '特征')
...
plt.tight_layout()
plt.savefig('rf_bayesian_feature_importance_chinese.png')
plt.close()
...
plt.plot(range(1, len(results) + 1), -results['mean_test_score'], 'o-', label='平均测试分数')
plt.axvline(x=best_index + 1, color='r', linestyle='--', label=f'最佳迭代 ({best_index + 1})')
set_plot_labels('贝叶斯优化学习曲线', '迭代次数', '负平均 R² 分数')
plt.legend(loc='best')
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('rf_bayesian_learning_curve_chinese.png')
plt.close()

# 创建实际值与预测值的材料科学风格图表（用于论文）
plt.figure(figsize=(10, 9))
plt.scatter(y_test, y_pred, alpha=0.6, edgecolor='k', c='royalblue', s=50)
plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'k--', lw=2)

# 添加误差范围线
plt.fill_between([y_test.min(), y_test.max()], 
                 [y_test.min() * 0.85, y_test.max() * 0.85], 
                 [y_test.min() * 1.15, y_test.max() * 1.15], 
                 alpha=0.2, color='gray', label='±15% 误差范围')

set_plot_labels('随机森林模型韧性预测性能', '实际韧性值 (J/cm²)', '预测韧性值 (J/cm²)')
...
plt.tight_layout()
plt.savefig('rf_material_science_plot_chinese.png', dpi=300, bbox_inches='tight')
plt.close()
...
# 创建性能对比柱状图
metrics = ['R²', 'RMSE', 'MAE']
original_values = [r2_orig, rmse_orig, mae_orig]
optimized_values = [r2, rmse, mae]

plt.figure(figsize=(12, 6))
x = np.arange(len(metrics))
width = 0.35

bars1 = plt.bar(x - width/2, original_values, width, label='原始随机森林', color='skyblue', edgecolor='black')
bars2 = plt.bar(x + width/2, optimized_values, width, label='贝叶斯优化随机森林', color='lightgreen', edgecolor='black')

plt.title('随机森林模型性能对比', fontsize=16)
...
plt.tight_layout()
plt.savefig('rf_model_comparison_chinese.png', dpi=300)
plt.close()
